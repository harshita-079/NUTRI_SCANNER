...
const scanImage = async (req, res) => {
  try {
    ...
    console.log("User ID:", req.body?.userId || req.user?._id || "anonymous");
    ...
    console.log("Product:", structuredData.product);
    console.log("Ingredients count:", structuredData.ingredients?.length);
    ...
    console.log("ABOUT TO SAVE SCAN");
...
const getScans = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Could not load scan history." });
  }
};
...
const getScanById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Could not load this scan." });
  }
};
